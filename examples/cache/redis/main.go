...
func main() {
	...
	redLock := redislock.NewRedLock(redisCli, redislock.WithPrefix(cfg.Prefix))
	...
	redisCache := cacheredis.NewRedisCache[string, *User](
		...
		cacheredis.WithLoader[string, *User](
			cacheloader.NewRedSyncLoader[string, *User](
				redLock, &loader{}, userKeyFunc, 10*time.Second,
			),
		),
	)
